...
dim(Training)
```
*Removing data that are near Zero*
```{r, echo=TRUE}
...
```
```{r, echo=TRUE}
predictRandForest <- predict(modRF1, newdata=testdata)
...
confMatGBM
```
FRom the analysis the accuracy of the Random Forest yeild best result with 100% accuracy while Decision Tree follows with 0.9998 and GMB is 0.9997

## Applying Best Model to Validate test Data
